...
背景画像は画像出力時に背景を含めるかどうかを選択できます。
位置ガイド画像は指定した背景画像のどの位置にキャラクターを置きたいかを示すと
キャラクター画像生成時に使用することが可能です。
基本的に背景画像と同じものを使用して、どの位置に置きたいかを追記したものを使ってください。
背景画像と解像度が違う場合は動作保証いたしかねます。
固定マスク画像（任意）は特に指定しなくていいです。
OKで登録、キャンセルで画面を閉じます。
...
作成したサブカテゴリをクリックすると、「キャラクター画像生成」ボタンが活性化するためクリックします。
表示されたビューに対してキャラクター画像を読み込ませ、切り抜きや拡大縮小、簡易背景透過、左右反転などを行います。
キャラクターの画像は背景が透過済みであることを想定した作りになっています。
なお簡易背景透過の精度は良くないので、前もって透過済みの画像を使用してもらうのがお勧めです。

![キャラクター画像作成](docs/images/editor-window.png)

#### 5. ビュー確認

透過されたキャラクター画像をキャラクター編集で選択すると、Custom Portraitビューに画像が表示されます。
ビュー画面を見ながらキャラクターの大きさや位置を調整してください。
ここで位置ガイド画像を登録している場合、ビューに薄っすらと表示させることが出来ます。
これを使うことで、キャラクターを出力する画像のどの位置におけばいいかを指し示すことが出来ます。
ガイドが表示されない場合はガイド濃度を高くしてください。
キャラクターの画像が大きい場合、キャラクター編集のビュー上のキャラサイズ調整バーを下げることで
キャラクターを小さくすることが出来ます。
また、Custom Portraitビューに表示されたキャラクターをドラッグ&ドロップすることで
ビュー内で位置調整を行うことが可能です。
...
PNG形式であれば透過済みの画像は透過したまま出力されます。
BMPファイル形式の場合は、Alphaマスクを自動生成して出力をすると
キャラクター部分を白く切り取った画像が出力されます。
ジェネレーションエクス等のbmpの画像ファイルとマスク画像がセットになったもので使用してください。
Alphaマスクは背景黒一色でしか試してないため、使用する場合は背景を黒としてください。
背景を含めるにチェックをすると、背景も含めてキャラクターが出力します。
背景を含めない場合は、背景に指定した解像度の透過された画像にキャラクターのみが出力されます。
出力先フォルダを指定しない場合、customport\customportrait\カテゴリ\サブカテゴリ\{連番}形式で出力されます。
基本的に最後に連番で出力したフォルダの番号を見て、それに対して+1したフォルダを作ります。
出力先フォルダを指定した場合は、その名前でフォルダを作成します。
