...
![カスタムポートレート作成](docs/images/editor-window.png)

#### 5. ビュー確認
- ビュー上でキャラクターが表示されている状態なら、マウスホイールで `5%` 単位の拡大縮小もできます。
- ビュー上のキャラクターやキャンバスをクリックしたあとに `←` / `→` キーを押すと、`1%` 単位でサイズを微調整できます。
- ビュー側でサイズを変えた場合も、キャラクター編集画面のスライダーと数値欄へ反映されます。
...
- キャラクター編集画面では、画像表示部分にマウスカーソルを合わせてホイールを回すと `5%` 単位で拡大縮小できます。
- キャラクター編集画面では、画像表示部分をクリックしたあとに `←` / `→` キーを押すと `1%` 単位で微調整できます。
- `マウスホイール` や `←` / `→` キーで変更した場合も、右側のスライダーと数値欄は連動して更新されます。
